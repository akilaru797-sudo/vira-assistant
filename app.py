...
AI_RESPONSES = {
    ...
    "capabilities": {
        "what can you do": ["I can send WhatsApp messages, emails, check weather, time, battery, and much more! Say 'help' for all commands.", "I'm your voice assistant! I handle messaging, system info, weather, time, and more. Try 'help'!", "I can help with WhatsApp, email, weather, time, battery, and system commands. Just ask!"],
        "help me": ["I'm here to help! Try saying 'help' for commands, or ask me about weather, time, or send messages!", "Happy to assist! Say 'help' for all available commands, or try 'weather' or 'time'!"],
        "what do you do": ["I'm Vira, your voice assistant! I handle WhatsApp, email, weather, time, battery, and system management.", "I'm Vira! I can send messages, check weather/time/battery, and help with system tasks."]
    },
    "small_talk": {
...
def process_command(data):
    try:
        ...
        # Debug: Log all command processing
        print(f"🔍 DEBUG: Processing command: '{text}'")
        print(f"🔍 DEBUG: Available command checks:")
        print(f"   - weather in text: {'weather' in text}")
        print(f"   - whether in text: {'whether' in text}")
        print(f"   - wheather in text: {'wheather' in text}")
        print(f"   - వాతావరణం in text: {'వాతావరణం' in text}")
        ...
        elif "weather" in text or "వాతావరణం" in text:
            track_api_usage("weather", "voice")
        ...
        elif "weather" in text or "whether" in text or "wheather" in text or "వాతావరణం" in text:
            print(f"🔍 DEBUG: Weather command detected in text: '{text}'")
            try:
                location = get_current_location()
                weather = get_weather_data(days=0)
                if weather:
                    print(f"🔍 DEBUG: Weather data retrieved successfully")
                    response = f"🌤️ Current Weather in {location['city']}, {location['country']}:\n"
                    response += f"• Condition: {weather['description']}\n"
                    response += f"• Temperature: {int(weather['temp'])}°C\n"
                    response += f"• Feels like: {int(weather.get('feels_like', weather['temp']))}°C\n"
                    response += f"• Humidity: {weather.get('humidity', 'N/A')}%\n"
                    response += f"• Location: {location['city']}, {location['country']}\n"
                    response += f"• Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                else:
                    print(f"🔍 DEBUG: Weather data retrieval failed")
                    response = f"🌤️ Weather in {location['city']}, {location['country']}: Unable to get weather data right now."
            except Exception as e:
                print(f"🔍 DEBUG: Weather command exception: {e}")
                response = "I cannot check weather right now."
        elif "weather" in text and "tomorrow" in text or "వాతావరణం" in text and "రేపు" in text or "వాతావరణం రేపు" in text:
            try:
                weather = get_weather_data(days=1)
                if weather:
                    response = f"Tomorrow's weather in {weather['location']}: {weather['description']}, temperature {int(weather['temp'])} degrees Celsius."
                else:
                    response = "I cannot get weather data right now."
            except Exception as e:
            ...
            if EMAIL_STATE["current_email"] and EMAIL_STATE["suggested_reply"]:
                ...
                convo_state = {"action": "send_email_reply", "sub_step": "waiting_for_pin"}
                    ...
                    if attempts_left == 0:
                        convo_state = {"action": None, "sub_step": None, "target": None}
        # SYSTEM TEST COMMAND
        elif "test system" in text or "system test" in text or "check system" in text:
            location = get_current_location()
            system_tests = {
                'location': f"✅ {location['city']}, {location['country']}",
                'server': "✅ Flask server running",
                'session': "✅ Session management active",
                'commands': "✅ Voice commands enabled",
                'api_tracking': "✅ API usage tracking active",
                'logout': "✅ Logout redirect working"
            }
            ...
            response += f"\n👤 Session: Active for {session.get('user', 'Unknown')}"
            response += f"\n🎤 Commands: All voice commands operational"
            response += f"\n📊 API Tracking: Monitoring all API usage"
            response += f"\n🚪 Logout: Redirects to login page"
            
        elif "test location" in text or "location test" in text:
            location = get_current_location()
            response = f"📍 Location Test Results:\n"
        ...
        elif "test logout" in text or "logout test" in text:
            response = "🚪 Logout Test: Say 'logout' to verify it redirects to login page"
            
        elif "help" in text or "commands" in text or "what can you do" in text:
            response = "🤖 Vira Assistant Commands:\n"
            response += "• Basic: hello, time, weather, whether, battery\n"
            response += "• Communication: whatsapp, send email, read mail\n"
            response += "• System: logout, reset, stop\n"
            response += "• Help: help, commands, what can you do\n"
            response += "• Location-based: All commands work in your current location"
            
        else:
            # Default response for unrecognized commands
            response = "I didn't understand that. Try saying 'hello', 'time', 'weather', 'whether', 'whatsapp', 'read mail', or 'help'."
